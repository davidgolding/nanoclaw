    ...
    try {
      ...
      if (this.enableE2ee) {
        try {
          ...
          const sdkCryptoStore = new RustSdkCryptoStorageProvider(
            cryptoStorePath,
            RustSdkCryptoStoreType.Sqlite,
          );
          if (this.deviceId) {
            await sdkCryptoStore.setDeviceId(this.deviceId);
          }
          cryptoStore = sdkCryptoStore;
        } catch (err) {
          logger.warn(
            { err },
...
registerChannel('matrix', (opts: ChannelOpts) => {
  ...
  const deviceId = process.env.MATRIX_DEVICE_ID || envVars.MATRIX_DEVICE_ID;
  return new MatrixChannel(homeserverUrl, accessToken, opts, e2ee, deviceId);
});
